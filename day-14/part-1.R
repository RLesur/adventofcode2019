...
reduce_required_once <- function(required) {
  ...
  for (i in seq.int(1, length.out = length(required_to_be_reduced))) {
    ...
    rest <- unlist(required_to_be_reduced[i]) %% reaction$quantity
    n_reaction <- unlist(required_to_be_reduced[i]) %/% reaction$quantity
    if (rest > 0) {
...
reduce_required <- function(required) {
  orders <- vapply(reactions[names(required)], function(x) x$order, integer(1))
  max_order <- max(orders)
  while (max_order > 1) {
    required <- reduce_required_once(required)
    orders <- vapply(reactions[names(required)], function(x) x$order, integer(1))
    max_order <- max(orders)
  }
  reduce_required_once(required)
}

reduce_required(required)
