...
Game <- R6::R6Class(
  ...
  public = list(
    ...
    get_score = function() {
      private$data[private$data$x == -1 & private$data$y == 0, ]$tile_id
    },
    print_game = function() {
      require(ggplot2)
      data <- private$data
      data <- data[!(data$x == -1 & data$y == 0), ]
      data$y <- -data$y
      ggplot(data, aes(x, y, fill = tile_id)) + 
        geom_tile()
    },
    compute_next_joystick_direction = function() {
      ball_x <- private$data[private$data$tile_id == 4, "x"]
      paddle_x <- private$data[private$data$tile_id == 3, "x"]
      if (ball_x < paddle_x) return(-1)
      if (ball_x > paddle_x) return(1)
      0
    },
    read_output = function(output) {
      output <- as.integer(strsplit(output, ",")[[1]])
      output <- matrix(data = output, ncol = 3, byrow = TRUE)
      colnames(output) <- c("x", "y", "tile_id")
      data <- private$data
      for (i in seq.int(1, length.out = nrow(output))) {
        data[data$x == output[i, "x"] & data$y == output[i, "y"], "tile_id"] <- output[i, "tile_id"]  
      }
      private$data <- data
      invisible(self)
    },
    resume = function() {
      input <- self$compute_next_joystick_direction()
      output <- private$computer$resume(input)
      #cat(output)
      self$read_output(output)
      invisible(self)
    },
    has_blocks = function() {
      any(private$data$tile_id == 2)
    },
    is_running = function() {
      private$computer$is_running() && self$has_blocks()
    },
    get_data = function() {
      private$data
    },
